(() => {
  ...
  const windyUrl=
    'https://embed.windy.com/embed2.html?lat=44.447&lon=12.013&zoom=8&level=surface'+
    '&overlay=rain&menu=&message=true&marker=true&calendar=now&pressure='+
    '&type=map&location=coordinates&detail=&detailLat=44.447&detailLon=12.013'+
  ...
  function loadWindy(force=false){
    ...
    if(force){
      f.src='about:blank';
      setTimeout(()=>{f.src=windyUrl},80);
    }else f.src=windyUrl;
    windyLoaded=true;
  }
  $('reloadWindy').addEventListener('click',()=>loadWindy(true));
